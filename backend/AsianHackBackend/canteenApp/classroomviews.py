...
def get_user_classroom(user):
    cls, _ = Classroom.objects.get_or_create(
        user=user, defaults={"name": "My AI Classroom"}
    )
    return cls
...
class ClassroomItemAddView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        video = get_object_or_404(
            CourseVideo, pk=request.data.get("video_id"), is_published=True
        )
        ...
        return Response(
            ClassroomItemSerializer(item).data, status=status.HTTP_201_CREATED
        )
...
class ClassroomItemProgressView(APIView):
    ...
    def patch(self, request, item_id, *args, **kwargs):
        ...
        if changed:
            item.save(
                update_fields=["progress_seconds", "completed", "last_watched_at"]
            )
