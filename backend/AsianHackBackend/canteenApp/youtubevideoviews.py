...
class RecommendedVideosView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        target_field_ids = list(
            set(field_ids_from_interests) | set(field_ids_from_skills)
        )
        ...
        qs = (
            qs.annotate(
                field_hits=Count(
                    "fields", filter=Q(fields__id__in=target_field_ids), distinct=True
                ),
                interest_hits=Count(
                    "interests",
                    filter=Q(interests__name__in=interest_names),
                    distinct=True,
                ),
                skill_hits=Count(
                    "skills", filter=Q(skills__name__in=skill_names), distinct=True
                ),
                keyword_hits=Count(
                    "keywords",
                    filter=Q(keywords__name__in=(interest_names + skill_names)),
                    distinct=True,
                ),
            )
            .annotate(
